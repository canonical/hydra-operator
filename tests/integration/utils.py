...
import jubilant
import yaml
from integration.constants import HYDRA_APP
...
def remove_integration(
    ...
    def _reintegrate() -> None:
        juju.integrate(f"{HYDRA_APP}:{integration_name}", remote_app_name)

    juju.remove_relation(f"{HYDRA_APP}:{integration_name}", remote_app_name)
    try:
        yield
    finally:
        _reintegrate()
